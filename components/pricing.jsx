...
//           subscriberType: "user",
//         });
//         // Update your front-end user dynamically
//         const updatedUser = await fetch(
//           "/api/convex/queries/getCurrentUser"
//         ).then((res) => res.json());
//         setUser(updatedUser); // or update your state management / context
//       }
//     } catch (error) {
...
//       planId: "cplan_316Pol6NP96JSJ3pxEKN51seQVp",
...
const PricingCard = ({
  ...
  const isCurrentPlan = currentUserPlan === id;
  ...
  const handlePopup = async () => {
    ...
    try {
      if (window.Clerk && window.Clerk.__internal_openCheckout) {
        ...
        // Update Convex database and front-end state
        // After checkout completes, update Convex DB
      //   const res = await fetch("/api/convex/mutations/upgradeToPro", {
      //   method: "POST",
      ...
      //console.error("Checkout error:", error);
      alert("please sign-in first, later you can upgrade.");
      //error=false;
       //window.Clerk.__internal_openCheckout=true;
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        <div className="text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent mb-6">
          ${price}
          {price > 0 && <span className="text-lg text-gray-400">/annual</span>}
        </div>
...
const PricingSection = () => {
  ...
  // Fetch current user plan from Convex
  useEffect(() => {
    const fetchUserPlan = async () => {
      if (!userId) return;
      const res = await fetch("/api/convex/queries/getCurrentUser");
      if (res.ok) {
  ...
  const plans = [
    ...
    },
    {
      ...
      featured: true,
      //planId: "cplan_316Pol6NP96JSJ3pxEKN51seQVp",
      planId: "cplan_33Cr4oeCmb0SeZtVa0iIfzomDW4",
      buttonText: "Upgrade to Pro",
    },
  ];
