...
//     //   }
//     // }
//     // Check plan limits based on user's subscription
//     if (user.plan === "free") {
//       // Free users: only 3 projects allowed
//       const projectCount = await ctx.db
//         .query("projects")
//         .withIndex("by_user", (q) => q.eq("userId", user._id))
//         .collect();
...
//       // (Optional) Add validation if subscription is expired
//       if (user.planExpiry && Date.now() > user.planExpiry) {
//         throw new Error("Your Pro subscription has expired. Please renew.");
//       }
//       // Otherwise, allow project creation
//     } else {
//       throw new Error("Invalid plan. Please contact support.");
//     }
  ...
  handler: async (ctx, args) => {
    ...
    if (user.plan === "free") {
      ...
      if (projectCount.length >= 100) {
        throw new Error(
          "Free plan limited to 100 projects. Upgrade to Pro for unlimited projects."
        );
      }
    } else if (user.plan === "pro") {
      if (user.planExpiry && Date.now() > user.planExpiry) {
      ...
      throw new Error("Invalid plan. Contact support.");
    }
...
export const deleteProject = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user || project.userId !== user._id) throw new Error("Access denied");
